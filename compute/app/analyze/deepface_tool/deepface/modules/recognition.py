...
def find(
    ...
    datastore_path = os.path.join(cache_dir, file_name)
    representations = []
    ...
    # 排除已经缓存的 _unregistered 条目
    representations = [rep for rep in representations if "_unregistered" not in rep.get("identity", "")]
    ...
    # 排除 _unregistered 目录下的图片，避免未识别人脸被当作注册库参考
    storage_images = {img for img in storage_images if "_unregistered" not in img}
