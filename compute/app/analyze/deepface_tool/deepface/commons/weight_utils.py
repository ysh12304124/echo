...
def download_weights_if_necessary(
    ...
    Weights are saved to the local weights/ directory instead of ~/.deepface/weights/.
    ...
    target_file = os.path.normpath(os.path.join(weights_dir, file_name))
    ...
    try:
        logger.info(f"\U0001f517 {file_name} will be downloaded from {source_url} to {target_file}...")
        ...
        raise ValueError(
            f"\u26d3\ufe0f\u200d\U0001f4a5 An exception occurred while downloading {file_name} from {source_url}. "
...
def load_model_weights(model: Sequential, weight_file: str) -> Sequential:
    ...
    Returns:
        model (keras.models.Sequential): pre-built model with updated weights
    """
    try:
