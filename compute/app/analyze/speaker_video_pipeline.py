...
def _identity_prototype(
    track_ids: Sequence[int],
    tracks: Sequence[FaceTrackLite],
) -> np.ndarray | None:
    descriptors = [tracks[index].descriptor for index in track_ids if tracks[index].descriptor is not None]
    if not descriptors:
        return None
    value = np.median(np.stack(descriptors), axis=0).astype(np.float32)
    norm = float(np.linalg.norm(value))
    return value / norm if norm > 1e-8 else value


def _select_active_identity_ids(
    ...
    *,
    ...
    """Choose visible identities without selecting two fragments of one person.

    Raw face tracking may fragment one real person into several identities.  In
    fixed-speaker mode we therefore select a *set* of identities jointly.  A
    combination is rewarded when its members are simultaneously visible (hard
    evidence that they are different people), have different appearance
    prototypes, cover many observations, and contain mouth-motion evidence.
    """
    ids = sorted(int(name.replace("可见人物", "")) for name in identity_tracks)
    if target_count is None or target_count <= 0 or target_count >= len(ids):
        return set(ids), {"method": "all", "candidate_ids": ids}
    target_count = max(1, min(int(target_count), len(ids)))

    strengths: dict[int, float] = {}
    time_bins: dict[int, set[int]] = {}
    prototypes: dict[int, np.ndarray | None] = {}
    for identity_id in ids:
        track_ids = identity_tracks[f"可见人物{identity_id}"]
        observations = [obs for track_id in track_ids for obs in tracks[track_id].observations]
        positive = sum(obs.speaking_score >= 0.45 for obs in observations)
        duration = sum(max(0.0, tracks[track_id].end - tracks[track_id].start) for track_id in track_ids)
        strengths[identity_id] = (
            sum(obs.speaking_score for obs in observations)
            + 0.06 * len(observations)
            + 0.35 * positive
            + 0.10 * duration
        )
        time_bins[identity_id] = {
            int(round(obs.time_sec * max(sample_fps, 0.5)))
            for obs in observations
        }
        prototypes[identity_id] = _identity_prototype(track_ids, tracks)

    max_strength = max(strengths.values(), default=1.0)
    best_combo: tuple[int, ...] | None = None
    best_score = -1e18
    combo_rows: list[dict[str, Any]] = []
    for combo in itertools.combinations(ids, target_count):
        strength_score = sum(strengths[item] / max(max_strength, 1e-6) for item in combo)
        co_visible_score = 0.0
        appearance_diversity = 0.0
        pair_count = 0
        for left, right in itertools.combinations(combo, 2):
            pair_count += 1
            left_bins, right_bins = time_bins[left], time_bins[right]
            common = len(left_bins & right_bins)
            denominator = max(1, min(len(left_bins), len(right_bins)))
            co_visible_score += min(1.0, common / denominator * 4.0)
            similarity = (_cosine_similarity(prototypes[left], prototypes[right]) + 1.0) / 2.0
            appearance_diversity += max(0.0, 1.0 - similarity)
        if pair_count:
            co_visible_score /= pair_count
            appearance_diversity /= pair_count
        score = strength_score + 3.0 * co_visible_score + 0.65 * appearance_diversity
        combo_rows.append({
            "ids": list(combo),
            "score": round(score, 6),
            "strength": round(strength_score, 6),
            "co_visible": round(co_visible_score, 6),
            "appearance_diversity": round(appearance_diversity, 6),
        })
        if score > best_score:
            best_score = score
            best_combo = combo
    selected = set(best_combo or tuple(ids[:target_count]))
    return selected, {
        "method": "joint_co_visibility_selection",
        "target_count": target_count,
        "candidate_ids": ids,
        "selected_ids": sorted(selected),
        "combinations": sorted(combo_rows, key=lambda item: item["score"], reverse=True)[:20],
    }
...
def analyze_video_faces(
    ...
    *,
    ...
    # Select visible candidates jointly.  This prevents two fragmented tracks
    # of the same foreground person from occupying two final speaker slots.
    active_identity_ids, active_selection = _select_active_identity_ids(
        dict(identity_tracks),
        tracks,
        target_count=target_speaker_count,
    ...
    diagnostics = {
        ...
        "active_identity_selection": active_selection,
    }
...
def _visual_voice_mapping(
    ...
    """Build one global one-to-one audio-speaker/visible-person assignment.

    The previous greedy vote could map only the strongest pair and leave the
    remaining face and voice as separate people.  This implementation builds a
    complete audiovisual evidence matrix and solves a small maximum-weight
    bipartite assignment.  When speaker and visible counts are equal, one strong
    anchor is enough to infer the complementary assignments without creating
    additional identities.
    """
    if visual is None or visual.identity_count == 0:
        return {}, {"votes": {}, "mapping": {}, "method": "disabled"}

    speakers = sorted({turn.speaker for turn in turns})
    identities = [f"可见人物{item}" for item in sorted(visual.active_identity_ids)]
    if not speakers or not identities:
        return {}, {"votes": {}, "mapping": {}, "method": "no_candidates"}

    votes: dict[tuple[str, str], float] = defaultdict(float)
    anchor_votes: dict[tuple[str, str], float] = defaultdict(float)
    for word in words:
        ...
        runner_up_visual = ranked_visual[1][1] if len(ranked_visual) > 1 else 0.0
        for speaker, audio_score in audio_scores.items():
            if speaker not in speakers or audio_score <= 0.02:
                continue
            for identity, visual_score in visual_scores.items():
                if identity not in identities or visual_score <= 0.02:
                    continue
                contribution = duration * (audio_score ** 1.35) * (visual_score ** 1.35)
                votes[(speaker, identity)] += contribution
        if top_visual >= 0.58 and top_visual - runner_up_visual >= 0.12:
            speaker, audio_score = max(audio_scores.items(), key=lambda item: item[1])
            if audio_score >= 0.45:
                anchor_votes[(speaker, top_identity)] += duration * audio_score * top_visual

    speaker_totals = {
        speaker: sum(votes.get((speaker, identity), 0.0) for identity in identities)
        for speaker in speakers
    }
    identity_totals = {
        identity: sum(votes.get((speaker, identity), 0.0) for speaker in speakers)
        for identity in identities
    }
    pair_confidence: dict[tuple[str, str], float] = {}
    for speaker in speakers:
        for identity in identities:
            value = votes.get((speaker, identity), 0.0)
            speaker_share = value / max(speaker_totals[speaker], 1e-9)
            identity_share = value / max(identity_totals[identity], 1e-9)
            anchor = anchor_votes.get((speaker, identity), 0.0)
            pair_confidence[(speaker, identity)] = float(np.clip(
                0.58 * speaker_share + 0.32 * identity_share + 0.10 * min(1.0, anchor),
                0.0,
                1.0,
            ))

    # Solve the rectangular maximum-weight assignment directly. Enumerating
    # all permutations is factorial in the number of visible identities and
    # can exhaust the host when a long video produces many candidates.
    score_matrix = np.array(
        [
            [
                pair_confidence[(speaker, identity)]
                + 0.035 * math.log1p(votes.get((speaker, identity), 0.0) * 100.0)
                for identity in identities
            ]
            for speaker in speakers
        ],
    ...
    best = {
        speakers[row]: identities[column]
        for row, column in zip(row_indices, column_indices)
    }

    def assignment_score(candidate: dict[str, str]) -> float:
        return sum(
            pair_confidence[(speaker, identity)]
            + 0.035 * math.log1p(votes.get((speaker, identity), 0.0) * 100.0)
            for speaker, identity in candidate.items()
        )
    anchor_pairs = {
        pair for pair, value in anchor_votes.items()
        if value >= 0.08 and pair_confidence.get(pair, 0.0) >= 0.46
    }
    equal_complete_case = len(speakers) == len(identities) and bool(anchor_pairs)
    mapping: dict[str, str] = {}
    for speaker, identity in best.items():
        value = votes.get((speaker, identity), 0.0)
        confidence = pair_confidence.get((speaker, identity), 0.0)
        speaker_share = value / max(speaker_totals[speaker], 1e-9)
        accept = (
            value > 0.0
            and (
                confidence >= 0.44
                or (equal_complete_case and confidence >= 0.18)
                or (len(speakers) == len(identities) == 1)
            )
            and (speaker_share >= 0.30 or equal_complete_case)
        )
        if accept:
            mapping[speaker] = identity

    return mapping, {
        "method": "global_one_to_one_assignment",
        "votes": {
            f"{speaker}->{identity}": round(votes.get((speaker, identity), 0.0), 6)
            for speaker in speakers for identity in identities
        },
        "anchor_votes": {
            f"{speaker}->{identity}": round(anchor_votes.get((speaker, identity), 0.0), 6)
            for speaker in speakers for identity in identities
        },
        "pair_confidence": {
            f"{speaker}->{identity}": round(pair_confidence[(speaker, identity)], 6)
            for speaker in speakers for identity in identities
        },
        "best_complete_assignment": best,
        "complete_assignment_score": round(assignment_score(best), 6) if best else 0.0,
        "complement_inference": equal_complete_case,
        "mapping": mapping,
    }
...
def reconcile_person_registry(
    segments: Sequence[SpeechSegment],
    visual: VisualAnalysis | None,
    fusion_diagnostics: dict[str, Any],
    speaker_relabel_mapping: dict[str, str],
    faces_dir: Path,
) -> dict[str, Any]:
    """Collapse audio labels and visual identities into one canonical registry.

    Final audio speakers are the only person IDs exposed to downstream code.
    A visual identity is an attribute/evidence source of that person, never an
    additional person.  Face files are renamed to the final speaker ID, and
    unmatched raw visual identities remain diagnostics only.
    """
    final_speakers = sorted({segment.speaker for segment in segments})
    raw_audio_to_visual = dict(fusion_diagnostics.get("audio_to_visual") or {})
    final_audio_to_visual = {
        speaker_relabel_mapping.get(raw_speaker, raw_speaker): visual_identity
        for raw_speaker, visual_identity in raw_audio_to_visual.items()
        if speaker_relabel_mapping.get(raw_speaker, raw_speaker) in final_speakers
    }
    visual_to_final = {visual_identity: speaker for speaker, visual_identity in final_audio_to_visual.items()}

    for segment in segments:
        raw_visual = segment.visual_person
        mapped_from_visual = visual_to_final.get(raw_visual or "")
        expected_visual = final_audio_to_visual.get(segment.speaker)
        if mapped_from_visual == segment.speaker or expected_visual is not None:
            segment.visual_person = segment.speaker
        else:
            segment.visual_person = None

    canonical_tracks: dict[str, list[int]] = {speaker: [] for speaker in final_speakers}
    raw_identity_tracks: dict[str, list[int]] = {}
    if visual is not None:
        raw_identity_tracks = dict(visual.identity_tracks)
        for speaker, visual_identity in final_audio_to_visual.items():
            canonical_tracks[speaker] = list(raw_identity_tracks.get(visual_identity, []))

    # Keep only one user-facing face file per canonical speaker.  Raw visible
    # identity images are diagnostic implementation details and are removed so
    # callers cannot count them as extra people.
    face_bytes: dict[str, bytes] = {}
    for speaker, visual_identity in final_audio_to_visual.items():
        source = faces_dir / f"{visual_identity}.jpg"
        if source.is_file():
            face_bytes[speaker] = source.read_bytes()
    for source in faces_dir.glob("*.jpg"):
        source.unlink()
    for speaker, payload in face_bytes.items():
        (faces_dir / f"{speaker}.jpg").write_bytes(payload)

    unmatched_visual = sorted(set(raw_identity_tracks) - set(visual_to_final))
    unmatched_audio = sorted(set(final_speakers) - set(final_audio_to_visual))
    registry = {
        "canonical_person_count": len(final_speakers),
        "canonical_person_ids": final_speakers,
        "audio_speaker_to_visual_identity": final_audio_to_visual,
        "visual_identity_to_audio_speaker": visual_to_final,
        "canonical_identity_tracks": canonical_tracks,
        "unmatched_audio_speakers": unmatched_audio,
        "unmatched_visual_identities_diagnostics_only": unmatched_visual,
        "policy": "audio speaker is canonical person id; visual identity is evidence only",
    }
    fusion_diagnostics["canonical_audio_to_visual"] = final_audio_to_visual
    fusion_diagnostics["person_registry"] = registry

    if visual is not None:
        visual.identity_count = len(final_speakers)
        visual.diagnostics["raw_identity_tracks"] = raw_identity_tracks
        visual.diagnostics["raw_identity_count"] = len(raw_identity_tracks)
        visual.diagnostics["identity_count"] = len(final_speakers)
        visual.diagnostics["identity_tracks"] = canonical_tracks
        visual.diagnostics["person_registry"] = registry
        (faces_dir / "manifest.json").write_text(
            json.dumps(visual.diagnostics, ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
    return registry
    ...
    *,
    ...
    payload = {
        ...
        "pipeline": {
            ...
            "visible_people": "GPU-first detector + internal tracks; raw visual IDs are diagnostics only",
            "voice_identity": "per-pyannote acoustic spectral prototypes",
            "identity_registry": "audio speaker is canonical; one-to-one visual binding; no audio/visual double counting",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="speaker_fusion_run_") as temp_name:
        ...
        person_registry = reconcile_person_registry(
            segments,
            visual,
            fusion_diagnostics,
            speaker_mapping,
        ...
        count_bounds["unified_person_count"] = person_registry["canonical_person_count"]
        count_bounds["mapped_visible_person_count"] = len(person_registry["audio_speaker_to_visual_identity"])
        logger.info(
            "unified person registry: persons=%d mapped_faces=%d unmatched_audio=%d raw_visual_diagnostics=%d",
            person_registry["canonical_person_count"],
            len(person_registry["audio_speaker_to_visual_identity"]),
            len(person_registry["unmatched_audio_speakers"]),
            len(person_registry["unmatched_visual_identities_diagnostics_only"]),
        )
