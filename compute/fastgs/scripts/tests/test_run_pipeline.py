...
def test_stage_command_has_explicit_four_stage_order(tmp_path):
    args = arguments(tmp_path)
    assert [stage_command(stage, args)[1].split("/")[-1] for stage in STAGES] == ["run_colmap.py", "align_colmap.py", "run_fastgs.py", "export_outputs.py"]
